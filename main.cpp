...
int main()
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            {//if my health decreasd or my mana is not enough to attack
            ...
            else if (controlCount < 4) {//CONTROL MODE
                //initalalize the position
                if (controlCount == 0)
                {
                    default_positions[0].first = 9000;
                    default_positions[0].second = 4500;
                    cout << "MOVE " << default_positions[0].first << " " << default_positions[0].second << endl;
                    controlCount++;
                }
                else {//enter here 3 times counter from 1 to 3 
                    for (auto it = Spiders.begin(); it != Spiders.end(); it++)
                    {
                        double myDist = sqrt(pow(ordered_heros[0].getX() - (it->second.getX()), 2) + pow(ordered_heros[0].getY() - (it->second.getY()), 2));
                        if (myDist <= 2200&&it->second.get_near_base()!=2)
                        {
            ...
            else {//attack => full health and enough mana and I already controlled 3 spiders
                //change initial position to the enemy base
                if (base_x == 0 && base_y == 0)
                {
                    default_positions[0].first = 13630;
                    default_positions[0].second = 6000;
                ...
                if (controlCount == 4)//MOVE TO THE OPPONENT BASE TO ATTACK IT
                {
                    cout << "MOVE " << default_positions[0].first << " " << default_positions[0].second << endl;
                    controlCount++;
                }
                    ...
                    if (my_mana >= 10)
                    {
                        for (auto it = Spiders.begin(); it != Spiders.end(); it++)
                        {
                            double spiderToEnemy = sqrt(pow(enemy_x - it->second.getX(), 2) + pow(enemy_y - it->second.getY(), 2));//distance between hero and enemy base
                            double myDist = sqrt(pow(ordered_heros[0].getX() - it->second.getX(), 2) + pow(ordered_heros[0].getY() - it->second.getY(), 2));//distance betweem hero and spider
                            if (it->second.getShieldLife() == 0&&it->second.get_near_base()==2)
                            {
                                if (myDist <= 1280) {
                                ...
                                else if (myDist <= 2200 && spiderToEnemy <= 5000) {//if the spider near to the opponent base shield it
                                    cout << "SPELL SHIELD " << it->second.getID() << endl;
                                    spellSpider = true;
                                    my_mana -= 10;
                                    break;
                                }
                    ...
                    if (!spellSpider && !hitHero)//if you can't spell or you there is no one to spell
                    {
                        bool isMovedToSpell = false;
                        for (auto it = Spiders.begin(); it != Spiders.end(); it++)
                        {
                           /* double dest_toAttack = sqrt(pow(enemy_x - (it->second.getX() + it->second.getVX()), 2) + pow(enemy_y - (it->second.getY() + it->second.getVY()), 2));*/
                            if (it->second.get_near_base()!=2)//move to the spider to spell it the next iteration
                            {
                                cout << "MOVE " << it->second.getX() << " " << it->second.getY() << endl;
                                isMovedToSpell = true;
                                break;
                            }
                        }
                        if (!isMovedToSpell)//lo mafi4 7ad m4 mohm aro7lo 34an a3mlo spell haro7 le al defualt position
                        {
                            cout << "MOVE " << default_positions[0].first << " " << default_positions[0].second << endl;
                        }
                    }
